...
public class SOM {
	...
	Layer outputLayer;
	int deltaR, deltaC;
	static final int numGenerations = 10;
	static final int numRows = 26;
	static final int numCols = 26;
	static final int numVectors = 100;
	int running_color = -3;
	...
	void run(){
		...
		inputLayer.readInput("/home/w/Dropbox/studia/sn/2/dane/cs-100.txt");
		outputLayer = new Layer(numCols, numCols);
		train(numVectors, numGenerations, 0xff0000, img); //red
		
		inputLayer = new Layer(numRows, numCols);
		inputLayer.readInput("/home/w/Dropbox/studia/sn/2/dane/de-100.txt");
		outputLayer = new Layer(numCols, numCols);
		train(numVectors, numGenerations, 0x00ff00, img); //green
		
		inputLayer = new Layer(numRows, numCols);
		inputLayer.readInput("/home/w/Dropbox/studia/sn/2/dane/en-100.txt");
		outputLayer = new Layer(numCols, numCols);
		train(numVectors, numGenerations, 0x0000ff, img); //blue
		
		inputLayer = new Layer(numRows, numCols);
		inputLayer.readInput("/home/w/Dropbox/studia/sn/2/dane/es-100.txt");
		outputLayer = new Layer(numCols, numCols);
		train(numVectors, numGenerations, 0x00ffff, img); //cyan
		
		inputLayer = new Layer(numRows, numCols);
		inputLayer.readInput("/home/w/Dropbox/studia/sn/2/dane/fr-100.txt");
		outputLayer = new Layer(numCols, numCols);
		train(numVectors, numGenerations, 0xff00ff, img); //magenta
		
		inputLayer = new Layer(numRows, numCols);
		inputLayer.readInput("/home/w/Dropbox/studia/sn/2/dane/pl-100.txt");
		outputLayer = new Layer(numCols, numCols);
		train(numVectors, numGenerations, 0xffff00, img); //yellow
		
		inputLayer = new Layer(numRows, numCols);
		inputLayer.readInput("/home/w/Dropbox/studia/sn/2/dane/sk-100.txt");
		outputLayer = new Layer(numCols, numCols);
	...
	double getDistance(int inputNeuronIndex){
		double sum = 0;
		
		// calculating output for output neuron
		for(int j = 0; j < inputLayer.neurons.length; ++j){
			sum += Math.pow(inputLayer.neurons[inputNeuronIndex] - outputLayer.weights[inputNeuronIndex][j], 2.0);
		}
	...
	int updateWeights(){
		...
		++outputLayer.neuronsTest[winner][winner];
		
		int currentUpdateCount = 0;
		for(int i = 0; i < numRows; ++i){			
			for(int j = 0; j < numCols; ++j){
				if(isNeighborhood(i, j, winner)){
	...
	boolean isNeighborhood(int r, int c, int w){
...
//				if(outputLayer.neuronsTest[i][j] == max_value){
				//if(outputLayer.neuronsTest[i][j] > 0.1 * max_value){
